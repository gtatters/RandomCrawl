...
Technically we want to replicate this 1000 to 10000 times to assess the empirical distribution, but the code is slow and best run using parallel computing.  Below we'll create two functions, one with $Q_{10}$=2 and the other with $Q_{10}$=1 and run 30 replicates for each, since the time required is very long:
